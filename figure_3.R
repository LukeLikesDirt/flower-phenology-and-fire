###############################################################################*
# ANALYSIS OF LIFE FORMS FLOWERING PATTERNS
###############################################################################*
...
# Read the raw data
data <- fread("data_lifeforms.txt") %>%
  # Fill missing combinations with 0
  complete(lifeform, month, site, fill = list(richness_peak_flowering = 0))
...
# Get unique life forms in the data
unique_lifeforms <- unique(data$lifeform)
print(paste("Life forms in data:", paste(unique_lifeforms, collapse = ", ")))
...
fit_glm_by_lifeform <- function(lifeform_value) {
  subset_data <- data %>% filter(lifeform == lifeform_value)
...
models_list <- list()
for(lifeform in unique_lifeforms) {
  models_list[[lifeform]] <- fit_glm_by_lifeform(lifeform)
}

# Check model assumptions
DHARMa::simulateResiduals(models_list$Chamaephyte, plot = TRUE)
DHARMa::simulateResiduals(models_list$Geophyte, plot = TRUE)
DHARMa::simulateResiduals(models_list$Hemicryptophyte, plot = TRUE)
DHARMa::simulateResiduals(models_list$Therophyte, plot = TRUE)
...
for(lifeform in names(models_list)) {
  results <- get_emmeans_lifeform(models_list[[lifeform]], lifeform)
  emmeans_results[[lifeform]] <- results$means
}
...
for(lifeform in names(models_list)) {
  cat("Bootstrapping effects for:", lifeform, "\n")
  lifeform_data <- data %>% filter(lifeform == !!lifeform)
  
  # Bootstrapping with error handling
  tryCatch({
    bootstrap_results[[lifeform]] <- bootstrap_glm_effects(
      models_list[[lifeform]],
      lifeform_data
    )
    cat("  Completed successfully:\n")
    cat("    ", bootstrap_results[[lifeform]]$n_successful, "successful samples\n")
    }, error = function(e) {
...
effects_data <- bind_rows(lapply(names(bootstrap_results), function(lifeform) {
  x <- bootstrap_results[[lifeform]]$effects
  x$lifeform <- lifeform
  x
})) %>%
  mutate(
...
density_data <- bind_rows(lapply(names(bootstrap_results), function(lifeform) {
  x <- bootstrap_results[[lifeform]]$density_data
  x$lifeform <- lifeform  # Explicitly add lifeform column
  x
})) %>%
  mutate(
    Month = factor(Month, levels = c("Oct", "Nov", "Dec", "Jan")),
...
intercept_data <- bind_rows(lapply(names(bootstrap_results), function(lifeform) {
    tibble(
      lifeform = lifeform,
      intercept = bootstrap_results[[lifeform]]$intercept
    )
  }))
...
final_plot <- plot_lifeform_means / plot_lifeform_effects
...
bootstrap_summary <- map_dfr(names(bootstrap_results), function(lifeform) {
  bootstrap_results[[lifeform]]$effects %>%
    mutate(lifeform = lifeform) %>%
    select(lifeform, Month, effect, lower, upper)
})

# Export results
fwrite(bootstrap_summary, "figure_3_bootstrap_summary.txt", sep = "\t")
fwrite(emmeans_combined, "figure_3_marginal_means.txt", sep = "\t")
